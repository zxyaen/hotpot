#!/usr/bin/env node
/**
 * 通过 mcporter 调用 CloudBase MCP 批量插入食材和锅底数据
 */
...
function insert(collectionName, doc) {
  const docStr = JSON.stringify(JSON.stringify(doc));
  const cmd = `mcporter call cloudbase.writeNoSqlDatabaseContent action=insert collectionName=${collectionName} documents='[${JSON.stringify(doc)}]'`;
  try {
...
async function main() {
  console.log(`开始插入 ${FOODS.length} 条食材数据...\n`);
  let ok = 0, fail = 0;
  for (const food of FOODS) {
    const r = insert('foods', food);
    if (r.error || r.success === false) {
      console.error(`❌ ${food.name}: ${r.error || JSON.stringify(r)}`);
      fail++;
    } else {
      console.log(`✅ ${food.emoji} ${food.name}`);
      ok++;
    }
  }
  console.log(`\n食材插入完成: ${ok} 成功, ${fail} 失败`);

  console.log(`\n开始插入 ${POTS.length} 条锅底数据...\n`);
  ok = 0; fail = 0;
  for (const pot of POTS) {
    const r = insert('pots', pot);
    if (r.error || r.success === false) {
      console.error(`❌ ${pot.name}: ${r.error || JSON.stringify(r)}`);
      fail++;
    } else {
      console.log(`✅ ${pot.emoji} ${pot.name}`);
      ok++;
    }
  }
  console.log(`\n锅底插入完成: ${ok} 成功, ${fail} 失败`);
}
