...
def insert(collection, doc):
    docs_json = json.dumps([doc], ensure_ascii=False)
    result = subprocess.run(
        ['mcporter', 'call', 'cloudbase.writeNoSqlDatabaseContent',
         f'action=insert', f'collectionName={collection}', f'documents={docs_json}'],
...
print(f"开始插入 {len(FOODS)} 条食材数据...\n")
ok = fail = 0
for food in FOODS:
    r = insert('foods', food)
    if r.get('success'):
        print(f"✅ {food['emoji']} {food['name']}")
        ok += 1
    else:
        print(f"❌ {food['name']}: {r}", file=sys.stderr)
        fail += 1
print(f"\n食材: {ok} 成功, {fail} 失败")
...
for pot in POTS:
    r = insert('pots', pot)
    if r.get('success'):
        print(f"✅ {pot['emoji']} {pot['name']}")
        ok += 1
    else:
        print(f"❌ {pot['name']}: {r}", file=sys.stderr)
        fail += 1
print(f"\n锅底: {ok} 成功, {fail} 失败")
print("\n✅ 全部完成！")
