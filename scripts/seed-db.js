/**
 * CloudBase 数据库初始化脚本
 * 用法：TCB_SECRET_ID=xxx TCB_SECRET_KEY=xxx node scripts/seed-db.js
 * 或在 .env 文件中配置后执行
 */
...
if (!SECRET_ID || !SECRET_KEY) {
  ...
  console.error('用法: TCB_SECRET_ID=xxx TCB_SECRET_KEY=xxx node scripts/seed-db.js');
  process.exit(1);
}
...
async function callTCBApi(action, params) {
  ...
  const signStr = sortedKeys.map(k => `${k}=${requestParams[k]}`).join('&');
  const hmac = crypto.createHmac('sha256', SECRET_KEY);
  hmac.update(`GET tcb.tencentcloudapi.com/?${signStr}`);
  requestParams.Signature = hmac.digest('base64');

  const query = Object.keys(requestParams).map(k => `${k}=${encodeURIComponent(requestParams[k])}`).join('&');
...
async function main() {
  console.log('开始插入食材数据...');
  
  // 插入食材（每条单独插入，避免超大请求）
  for (let i = 0; i < FOODS.length; i++) {
    const food = FOODS[i];
    const result = await callTCBApi('DatabaseAdd', {
      CollectionName: 'foods',
      QueryType: 'ADD',
      Query: JSON.stringify(food)
    });
    if (result.Response && result.Response.Error) {
      console.error(`❌ 插入 ${food.name} 失败:`, result.Response.Error.Message);
    } else {
      console.log(`✅ [${i+1}/${FOODS.length}] ${food.emoji} ${food.name}`);
    }
  }
  ...
  for (let i = 0; i < POTS.length; i++) {
    ...
    const result = await callTCBApi('DatabaseAdd', {
      CollectionName: 'pots',
      QueryType: 'ADD',
      Query: JSON.stringify(pot)
    });
    if (result.Response && result.Response.Error) {
      console.error(`❌ 插入 ${pot.name} 失败:`, result.Response.Error.Message);
    } else {
      console.log(`✅ [${i+1}/${POTS.length}] ${pot.emoji} ${pot.name}`);
    }
  }
